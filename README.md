...
- **如何保证数据一致性？** 在记录饮食时采用了“食物名称快照”机制，防止因食物库字典修改而导致的历史记录数据混乱。
- **如何处理复杂的跨表统计？** 采用了门面模式的思想，通过 `AnalysisService` 编排 `DietRecord`、`NutritionGoal` 和 `HealthData` 多个模块的数据，一站式返回综合分析结果。
- **如何保证前端体验？** 通过 Vite Proxy 解决了跨域问题，并利用 Axios 拦截器实现了全局的登录态校验与错误弹窗提示。
